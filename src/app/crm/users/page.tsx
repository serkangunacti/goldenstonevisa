      ...
      <div className="p-4 md:p-6 max-w-4xl mx-auto">
        <div className="flex items-start justify-between mb-6 gap-3">
          ...
          <div className="flex gap-2 flex-shrink-0">
            ...
            <button onClick={openAdd} className="flex items-center gap-1.5 px-3 py-2 bg-indigo-600 text-white rounded-xl text-sm font-medium hover:bg-indigo-700 transition shadow">
              <Plus size={15} /> <span className="hidden sm:inline">Yeni Kullanıcı</span><span className="sm:hidden">Ekle</span>
            </button>
          </div>
        </div>
        ...
        <div className="md:hidden">
          {loading ? (
            ...
            <div className="text-center py-12 text-gray-400 bg-white rounded-2xl border border-gray-100">Kullanıcı bulunamadı</div>
          ) : (
            <div className="space-y-3">
              {users.map((u) => (
                <div key={u.uid} className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="flex flex-col items-end gap-2 flex-shrink-0">
                      ...
                      <div className="flex gap-1">
                        <button onClick={() => openEdit(u)} className="p-1.5 text-indigo-600 hover:bg-indigo-50 rounded-lg transition">
                          <Edit2 size={14} />
                        </button>
                        <button onClick={() => toggleDisable(u)} className={`p-1.5 rounded-lg transition ${u.disabled ? "text-green-600 hover:bg-green-50" : "text-amber-600 hover:bg-amber-50"}`}>
                          {u.disabled ? <UserCheck size={14} /> : <UserX size={14} />}
                        </button>
                        <button onClick={() => setDeleteUid(u.uid)} className="p-1.5 text-red-500 hover:bg-red-50 rounded-lg transition">
                          <Trash2 size={14} />
                        </button>
                      </div>
                    </div>
