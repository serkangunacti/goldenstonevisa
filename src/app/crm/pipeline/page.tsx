    ...
    <CrmShell>
      <div className="p-6 max-w-full">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Satış Pipeline</h1>
            ...
            return (
              <div key={stage} className="flex-shrink-0 w-72">
                ...
                <div className="space-y-3 min-h-[200px]">
                  {stageDeals.map((d) => (
                    <div key={d.id} className={`bg-white rounded-xl border-l-4 ${STAGE_COLORS[stage]} p-4 shadow-sm`}>
                      ...
                      {d.kapanisTarihi && (
                        <p className="text-xs text-gray-400 mt-2">Kapanış: {d.kapanisTarihi}</p>
                      )}
                      {d.sorumlu && (
                        <p className="text-xs text-gray-400">Sorumlu: {d.sorumlu}</p>
                      )}
                    </div>
                  ))}
                </div>
